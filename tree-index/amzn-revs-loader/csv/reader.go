...
func (r *Reader) readRecord(dst []string) ([]string, error) {
	...
	// "があったらエスケープのためにbackslash付与
	//fmt.Println(string(line))
	escapes := regexp.MustCompile(`[^\\]"`)
	if i := escapes.FindAllIndex(line, -1); i != nil {
		fmt.Println("Find unescapeed double quote")
		//fmt.Println(j)
		for j := range i {
			//fmt.Println(j)
			pt := i[j][1] + j*len(`\`) - 1
			line = append(line[:pt], append([]byte(`\`), line[pt:]...)...)
		}
	}
	//fmt.Println(string(line))
...
parseField:
	for {
		...
		if len(line) == 0 || line[0] != '"' {
			...
			if !r.LazyQuotes {
				...
				if j := bytes.IndexByte(field, '"'); j >= 0 && field[j-quoteLen] != '\\' {
	...
	if r.FieldsPerRecord > 0 {
		if len(dst) != r.FieldsPerRecord && err == nil {
			...
			fmt.Println("dst: ")
			fmt.Println("r.fieldIndexes: ", r.fieldIndexes)
			fmt.Println("r.FieldPerRecord: ", r.FieldsPerRecord)
